...
st.title("流行ごとのデータ分析")
st.write('新型コロナウィルスは定期的に変異を繰り返しながら、流行と収束を繰り返しています。その周期は4ヶ月ほどで、各々の流行における重なりが少ないことが特徴です。このため特定期間のデータを抽出することで、それぞれの変異株の特徴を分析できると考えられます。このページは特に流行時期を指定し、流行毎のデータ分析を行います。特に、ワクチンの普及によるオミクロン株への変異による感染力の飛躍的増加が、時系列データに与えた影響が顕著です。どうぞご活用ください。')
